...
def calculate_zigzag(high, low, deviation=0.002): # Tighter deviation (0.2%)
    n = len(high)
...
def main():
    ...
    # 4. Calculate ZigZag Peaks and Troughs (0.2% deviation)
    print("ZigZag tepeleri ve dipleri hesaplanıyor (0.2% sapma)...")
    last_peaks, last_troughs = calculate_zigzag(df['high'].values, df['low'].values, deviation=0.002)
    ...
    for i in range(1, len(df)):
        ...
        if not in_pos:
            # LONG: Yesterday Green, Close in [d_low - d_range, d_low], Close crosses above last_peak
            ...
            # SHORT: Yesterday Red, Close in [d_high, d_high + d_range], Close crosses below last_trough
    ...
    print("\n" + "=" * 60)
    print("ZIGZAG MSB 0.2% + GRID PROJEKSİYON STRATEJİSİ | BTC 15M (1 YIL)")
    print("=" * 60)
